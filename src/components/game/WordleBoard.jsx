...
const WordleBoard = ({ guesses, currentGuess, wordLength, maxGuesses }) => {
    const emptyRows = Array(maxGuesses - guesses.length - (currentGuess ? 1 : 0)).fill(null);
    ...
    const renderEmptyRow = (index) => {
        ...
        return (
            <div key={`empty-${index}`} className="guess-row">
                {letters.map((letter, letterIndex) => renderLetter(letter, null, letterIndex))}
            </div>
        );
    };
